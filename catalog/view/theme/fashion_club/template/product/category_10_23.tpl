		...
		foreach ($sorts as $sorts) { ?>
	       ...
	       <li><input type="radio" name="price" id="low" value="<?php echo $sorts['href']; ?>" onchange="location = this.value;">
			...
			if( ($sortcnt==5) || ($sortcnt==6) ){
				if( $sortcnt==5){
				echo '</ul></div><div class="sort"><h2>Sort by Popularity</h2><ul class="category_list">';
				}
			?>

					<?php if ($sorts['value'] == $sort . '-' . $order) { ?>
			        <li><input type="radio" name="popularity" id="high" value="<?php echo $sorts['href']; ?>" checked="checked" onchange="location = this.value;">
					<label for="high">
			        <?php 
					if ($sortcnt==5) echo "Low to High";
					if ($sortcnt==6) echo "High to Low";
					?></label></li>
			        <?php } else { ?>
			        
			       <li><input type="radio" name="popularity" id="low" value="<?php echo $sorts['href']; ?>" onchange="location = this.value;">
					<label for="low">
			        <?php 
					if ($sortcnt==5) echo "Low to High";
					if ($sortcnt==6) echo "High to Low";
