...
class ControllerCheckoutCart extends Controller {
	...
	public function index() {
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts(); //where does this function come from???
			
			//var_dump($products); 
			//why doesn't it get all the stuff in the array?
      		...
      		foreach ($products as $product) {
        		...
        		$this->data['products'][] = array(
					...
					'price'    => $price,
					'total'    => $total,
					'href'     => $this->url->link('product/product', 'product_id=' . $product['product_id'] . '&path=' .$product['cat_id']), //need to insert path here
					'remove'   => $this->url->link('checkout/cart', 'remove=' . $product['key'])
				);
      		}
			
			//echo "<script>console.log('category: ".$product['cat_id']."')</script>";
			// Gift Voucher
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/error/empty_basket.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/error/empty_basket.tpl';
			} else {
