<?php echo $header; ?>
					    	...
					    	<div id="language<?php echo $language['language_id']; ?>">
					    		<table class="form">
									<tr>
										...
										<td>
											<input type="text" name="article_description[<?php echo $language['language_id']; ?>][article_title]" value="<?php echo isset($article_description[$language['language_id']]['article_title']) ? $article_description[$language['language_id']]['article_title'] : ''; ?>" size="100" />
									...
									<tr>
										...
										<td>
											<textarea name="article_description[<?php echo $language['language_id']; ?>][description]" id="description<?php echo $language['language_id']; ?>"><?php echo isset($article_description[$language['language_id']]['description']) ? $article_description[$language['language_id']]['description'] : ''; ?></textarea>
									...
									<tr>
										...
										<td>
											<textarea name="article_description[<?php echo $language['language_id']; ?>][meta_description]" cols="200" rows="7"><?php echo isset($article_description[$language['language_id']]['meta_description']) ? $article_description[$language['language_id']]['meta_description'] : ''; ?></textarea>
										</td>
									</tr>
									...
									<tr>
										...
										<td>
											<textarea name="article_description[<?php echo $language['language_id']; ?>][meta_keyword]" cols="200" rows="7"><?php echo isset($article_description[$language['language_id']]['meta_keyword']) ? $article_description[$language['language_id']]['meta_keyword'] : ''; ?></textarea>
										</td>
									</tr>									
								</table>
					    	</div>
 					...
 					<div id="tab-option">
 						<table class="form">
 							...
 							<tr>
					            ...
					            <td>
					            	<input type="text" name="author_name" value="<?php echo $author_name; ?>" size="100" />
					       	...
					       	<tr>
					            ...
					            <td>
					            	<input type="text" name="keyword" value="<?php echo $keyword; ?>" size="100" />
				            ...
				            <tr>
				            	...
				            	<td>
				            		<div class="image"><img src="<?php echo $thumb2; ?>" alt="" id="thumb2" /><br />
					                	<input type="hidden" name="featured_image" value="<?php echo $featured_image; ?>" id="featured_image" />
					                  	<a onclick="image_upload('featured_image', 'thumb2');"><?php echo $text_browse; ?></a> | <a onclick="$('#thumb2').attr('src', '<?php echo $no_image; ?>'); $('#featured_image').attr('value', '');"><?php echo $text_clear; ?></a>
				                  	</div>
								</td>
				            </tr>
 						</table>
	 							...
	 							<tbody id="additional-description<?php echo $module_row; ?>">
	 								<tr>
	 									<td class="left">
	 										<?php foreach ($languages as $language) { ?>
	 											<div id="">
	 												<textarea name="article_addition_description[<?php echo $module_row ?>][<?php echo $language['language_id']; ?>][additional]" id="description-<?php echo $module_row; ?>-<?php echo $language['language_id']; ?>"><?php echo $additional_description[$language['language_id']]['additional']; ?></textarea>
	 												<img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" align="top" />
	 											</div>
	 										<?php } ?>
	 									</td>
	 									<td class="right"><a onclick="$('#additional-description<?php echo $module_row; ?>').remove();" class="button"><?php echo $button_remove; ?></a></td>
	 								</tr>
	 							</tbody>
	 							<?php $module_row++; ?>
	 						<?php } ?>
 					...
 					<div id="tab-related">
 							...
 							<tr id="product-wise" style="display:none">
 								...
 								<td>
 									<table>
	 									<tr>
	 										<td>
	 											<?php echo $entry_productwise; ?><br />
	 											<input type="text" name="product" value="" />
	 										</td>
	 										<td>&nbsp;&nbsp;</td>
	 										<td> 
	 											<div class="scrollbox" id="product-wise-list">
	 												<?php $class = 'odd'; ?>
	 												<?php if(isset($products)) { ?>
	 													<?php foreach($products as $product) { ?>
	 														<?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
	 														<div id="product-wise-list<?php echo $product['product_id']; ?>" class="<?php echo $class; ?>"> <?php echo $product['name']; ?><img src="view/image/delete.png" />
 						...
 						<table id="related-article" class="list">
							...
							<?php foreach($blog_related_articles as $related_articles) { ?>
								<tbody id="article-row<?php echo $article_row; ?>">
									<tr>
										...
										<td class="left"><a onclick="$('#article-row<?php echo $article_row; ?>').remove();" class="button"><?php echo $button_remove; ?></a></td>										
									</tr>
								</tbody>								
								<?php $article_row++; ?>
							<?php } ?>
							...
							<tfoot>
	            				<tr>
	              					<td colspan="3"></td>
	              					<td class="left"><a onclick="addArticles();" class="button"><?php echo $button_add_articles; ?></a></td>
	            				</tr>
	          				</tfoot>
 						</table>
	...
	<script type="text/javascript">
		...
		function addDescription() {
			...
			html += '		<td class="left">';
								<?php foreach ($languages as $language) { ?>
									html += '<div id="">'
									html += '	<textarea name="article_addition_description[' + module_row + '][<?php echo $language['language_id']; ?>][additional]" id="description-' + module_row + '-<?php echo $language['language_id']; ?>"></textarea> <img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" align="top" />';
									html += '</div>';
								<?php } ?>
			html += '		</td>';
			html += '		<td class="right"><a onclick="$(\'#additional-description' + module_row + '\').remove();" class="button"><?php echo $button_remove; ?></a></td>';
			html += '	</tr>';
			html += '</tbody>';
	...
	<script type="text/javascript">
		$('input[name=\'author_name\']').autocomplete({
			...
			source: function(request, response) {
				$.ajax({
					url: 'index.php?route=simple_blog/author/autocomplete&token=<?php echo $token; ?>&author_name=' +  encodeURIComponent(request.term),
					dataType: 'json',
			...
			select: function(event, ui) {
				...
				$('input[name=\'simple_blog_author_id\']').val(ui.item.value);
	...
	<script type="text/javascript">
		$('input[name=\'product\']').autocomplete({
			...
			select: function(event, ui) {
				...
				$('#product-wise-list').append('<div id="product-wise-list' + ui.item.value + '">' + ui.item.label + '<img src="view/image/delete.png" /><input type="hidden" name="product_wise[]" value="' + ui.item.value + '" /></div>');
	...
	<script type="text/javascript">
		...
		function addArticles() {
			...
			html += '	<tr>';
			html += '		<td class="left">';
			html += '			<input type="text" name="blog_related_articles[' + article_row + '][article_title]" value="" id="article-title-' + article_row + '" onkeyup="getArticles(' + article_row + ', this.value);" /> <input type="hidden" name="blog_related_articles[' + article_row + '][simple_blog_article_related_id]" value="0" />';
			html += '		</td>';			
			html += '		<td class="left">';
			...
			html += '			</select>';
			html += '		</td>';			
			html += '		<td class="left"><a onclick="$(\'#article-row' + article_row + '\').remove();" class="button"><?php echo $button_remove; ?></a></td>';
			html += '	</tr>';
			html += '</tbody>';
	...
	<script>
		function getArticles(article_row, value) {
			$('input[name=\'blog_related_articles[' + article_row + '][article_title]\']').autocomplete({
				...
				source: function(request, response) {
					$.ajax({
						url: 'index.php?route=simple_blog/simple_article/autocomplete_article&token=<?php echo $token; ?>&simple_blog_article_id=<?php echo $simple_blog_article_id; ?>&filter_name=' +  encodeURIComponent(request.term),
						dataType: 'json',
						success: function(json) {		
							response($.map(json, function(item) {
								return {
				...
				select: function(event, ui) {
					...
					$('input[name=\'blog_related_articles[' + article_row + '][simple_blog_article_related_id]\']').val(ui.item.value);
