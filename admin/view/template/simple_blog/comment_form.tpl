<?php echo $header; ?>
 					...
 					<div id="tab-general">
 						<table class="form">
 							...
 							<tr>
 								...
 								<td>
 									<input type="text" name="article_title" value="<?php echo $article_title; ?>" size="100" />
 					...
 					<div id="tab-comment">
 						<table class="form">
 							<tr>
 								...
 								<td>
 									<textarea name="comment" cols="200" rows="7"><?php echo $comment; ?></textarea>
 						...
 						<table id="comment-reply" class="list">
							...
							<?php foreach ($comment_reply as $reply) { ?>
	 							<tbody id="reply-<?php echo $module_row; ?>">
	 								<tr>
	 									...
	 									<td class="left">
	 										<textarea name="comment_reply[<?php echo $module_row; ?>][comment]"><?php echo $reply['comment']; ?></textarea>
	 										<?php if(isset($error_reply_comment[$module_row])) { ?>
	 											<span class="error"><?php echo $error_reply_comment[$module_row]; ?></span>
	 										<?php } ?>
	 									</td>
	 									<td class="left">
	 										<select name="comment_reply[<?php echo $module_row; ?>][status]">
	 									...
	 									<td class="left"><a onclick="$('#reply-<?php echo $module_row; ?>').remove();" class="button"><?php echo $button_remove; ?></a></td>
	 								</tr>
	 							</tbody>
	 							<?php $module_row++; ?>
	 						<?php } ?>
							...
							<tfoot>
	            				<tr>
	              					<td colspan="3"></td>
	              					<td class="left"><a onclick="addReply();" class="button"><?php echo $button_add_reply; ?></a></td>
	            				</tr>
	          				</tfoot>
	...
	<script type="text/javascript">
		...
		function addReply() {
			...
			html += '		</td>';
			html += '		<td class="left">';
			html += '			<textarea name="comment_reply[' + module_row + '][comment]"></textarea>';								
			html += '		</td>';
			html += '		<td class="left">';
			html += '			<select name="comment_reply[' + module_row + '][status]">';
			html += '				<option value="1"><?php echo $text_enabled; ?></option>';
			html += '				<option value="0"><?php echo $text_disabled; ?></option>';
			html += '			</select>';								
			html += '		</td>';
			html += '		<td class="left"><a onclick="$(\'#reply-' + module_row + '\').remove();" class="button"><?php echo $button_remove; ?></a></td>';
			html += '	</tr>';
			html += '</tbody>';
	...
	<script type="text/javascript">
		$('input[name=\'article_title\']').autocomplete({
			...
			source: function(request, response) {
				$.ajax({
					url: 'index.php?route=simple_blog/article/autocomplete&token=<?php echo $token; ?>&article_name=' +  encodeURIComponent(request.term),
					dataType: 'json',
